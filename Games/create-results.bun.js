...
for await (const item of parseNdJson(res)) {
  const { link, date, venue, teams, linescore, boxscore, players, flags, } = item;
  ...
  const { firstRun, isWalkOff, isExtraInnings, hadComeback } = flags;

  data.push({
    date, away, home, venue: venue.boxscoreName, firstRun, isWalkOff, isExtraInnings, hadComeback, link,
  })
}
